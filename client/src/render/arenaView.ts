...
function buildLightTower(x: number, z: number, baseY: number): LightTower {
  ...
  // REAL floodlight — the SAME lighting system as the sun: a SpotLight from the
...
export function createArenaView(radius = 28, lighting?: WorldLighting): ArenaView {
  ...
  // Each tower's SpotLight is a real shadow-caster (same lighting system as the
  // sun) added to the SCENE, so it lights the beans/ball and casts their shadows
  // onto the pitch at night.
  ...
  for (let row = 0; row < ROWS; row++) {
    ...
    const perRow = Math.floor((Math.PI * 2 * rr) / 1.25) // fewer, roomier fans
    for (let i = 0; i < perRow; i++) {
      ...
      seats.push({
        ...
        scale: 1.0 + Math.random() * 0.12, // full player size
