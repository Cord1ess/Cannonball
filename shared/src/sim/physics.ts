...
export function makeBall(): BallSim {
  return { x: 0, y: 3, z: 0, vx: 0, vy: 0, vz: 0 }
}

export function makePlayer(seat: number, x: number, z: number, yaw: number): PlayerSim {
  return { seat, x, y: 0, z, vx: 0, vy: 0, vz: 0, yaw, grounded: true, headerCd: 0 }
}
...
export function stepPlayer(p: PlayerSim, input: PlayerInputFrame, arena: Arena, dt: number): void {
  ...
  if (hit) {
    // kill outward velocity, keep the slide
...
/**
 * Aerial contact = HEADER (the signature move): a strong impulse along facing.
 * Grounded contact = weak body nudge. No button — jump into the ball.
 */
export function interactBallPlayers(
  ...
  for (const p of players) {
    ...
    if (!p.grounded && p.headerCd <= 0 && dist < HEADER_RANGE) {
      // HEADER: replace ball velocity with a facing-directed rocket
